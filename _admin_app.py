...
def get_price_var_min_max_last(ticker_yf: str):
    """
    Retorna:
     - close_today: fechamento mais recente ajustado
     - var: variação percentual diária
     - min_sexta: mínima de todas as sextas desde 2024-06-01
     - max_sexta: máxima de todas as sextas desde 2024-06-01
     - fechamento_mais_recente: último fechamento de sexta-feira
    """
    try:
        ...
        fechamento_mais_recente = (
            round(sextas["Close"].iloc[-1], 2) if not sextas.empty else None
        )
...
def main():
    try:
        ...
        # Prepara tickers para yfinance
        df["Ticker_YF"] = df["Ticker"].astype(str).str.strip() + ".SA"

        # Busca preços e estatísticas
        df[[
            "Cotação atual",
            "Var",
        ...
        def calcular_sr(row):
            H, L, C = (row.get("Máxima sexta desde jun/24"),
                       row.get("Mínima sexta desde jun/24"),
                       row.get("Fechamento mais recente"))
            if pd.notnull(H) and pd.notnull(L) and pd.notnull(C):
                P = (H + L + C) / 3
                return pd.Series([
                    round(L - 2*(H-P), 2), round(P-(H-L), 2), round(2*P-H, 2),
                    round(P, 2), round(2*P-L, 2), round(P+(H-L), 2), round(H+2*(P-L), 2)
                ])
        ...
        def encontrar_valores_proximos(row):
            ...
            niveis = sorted([row.get(k) for k in ["S3","S2","S1","P","R1","R2","R3"] if pd.notnull(row.get(k))])
        ...
        df["Amplitude"] = df.apply(
            lambda r: round(((r.get("Nível acima")/r.get("Nível abaixo")-1)*100), 2) 
            if pd.notnull(r.get("Nível abaixo")) and r.get("Nível abaixo")!=0 else None,
            axis=1
        )

        # Cálculo dos K-dividers
        k_div = [-2,-3,-5,-9,-17,-33,-65,65,33,17,9,5,3,2]
        k_cols = [f"K ({k})" for k in k_div]
        df[k_cols] = df["Amplitude"].apply(
            lambda amp: pd.Series([round(amp/k, 2) if pd.notnull(amp) else None for k in k_div])
        )

        # Encontra variações de faixa
        ...
        df["Spread (%)"] = df.apply(
            lambda r: round(r.get("Var (acima)") - r.get("Var (abaixo)"), 2)
            if pd.notnull(r.get("Var (abaixo)")) else None,
            axis=1
        )
        ...
        # Previsão de alvo por regressão linear
        today = date.today()
        wd = today.weekday()
        offset = (4 - wd) % 7 or 7
        ...
        # Oculta colunas conforme arquivo de configuração
        ocultar = [col for col in hidden_cols if col in df.columns]
        display_df = df.drop(columns=ocultar, errors="ignore")

        # Reordena colunas colocando Cotação após Ticker
        ...
        date_cols_fmt = sorted([c for c in display_df.columns if c[:4].isdigit() and "-" in c])[-5:]
        ...
        def highlight_colunas_comparadas(row):
            ...
            for i in range(1, len(vals)):
                ...
                if pd.notnull(ant) and pd.notnull(atual):
                    styles[i] = 'color: green; font-weight: bold' if atual > ant else 'color: red; font-weight: bold'
            return styles
